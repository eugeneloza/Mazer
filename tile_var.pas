 ...
 CalculateTileFaces;
  ...
  for i:=1 to maxTilesTypes do begin
    ...
    if Tiles[i].tilesizez=1 then
      try
        Tiles[i].Tile_PNG[1]:=LoadImage(models_folder+inttostr(i)+'.png', [TRGBAlphaImage]) as TRGBAlphaImage
      except
    ...
    for j:=1 to Tiles[i].tilesizez do
          try
            Tiles[i].Tile_PNG[j]:=LoadImage(models_folder+inttostr(i)+'_'+inttostr(j)+'.png', [TRGBAlphaImage]) as TRGBAlphaImage;
